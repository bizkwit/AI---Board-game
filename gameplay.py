...
def validate_move_string(input_s):
...
def get_valid_input(message=""):
    input_s = get_input_as_list(message)
    is_valid_input = False
    while not is_valid_input:
        is_valid_input = validate_move_string(input_s)
        if not is_valid_input:
            if game.is_file_input:
                game.is_file_input = False
                print("Input: is not valid.")
                print("Next moves will be done in manual mode.")
            input_s = get_input_as_list("Input not valid, try again: ")
        else:
            return input_s
